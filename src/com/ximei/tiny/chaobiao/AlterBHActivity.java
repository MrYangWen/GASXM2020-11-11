...
public class AlterBHActivity extends Activity {
	...
	protected void onCreate(Bundle paramBundle) {
		...
		if (shouhoutype.equals("qym")) {
			this.shouhouhint.setText("区域码操作");
			this.input1.setHint("请输入频率");
			this.input2.setHint("请输入区域码");
			oldmeter.setText("查询");
			newmeter.setText("修改");
			oldmeter.setChecked(true);
		}
...
//						input2hex = Integer.toHexString(input2nub);				
						String Addr1=localGetmsgID.CheckMeterID(input1msg);
						String Addr2=localGetmsgID.CheckMeterID(input2msg);
...
//						AlterBHActivity.this.input2hex = Integer.toHexString(input2nub);
					...
					}else if(shouhoutype.equals("qym")) {
						if(oldmeter.isChecked()) {
							CRCmsg = ("09"+ "00000000000000"+ "FF00");
						}else if(newmeter.isChecked()) {
							String pl = input1msg;
							Log.e("频率：", pl);
							//StrToHex thx = new StrToHex();
							pl = Integer.toHexString(Integer.parseInt(pl)).toUpperCase();
							if(pl.length()%2!=0) {
								pl="0"+pl;
							}
							Log.e("频率：", pl);
							pl = pl.substring(4)+pl.substring(2,4)+pl.substring(0,2);
							String qym = input2msg;
							Log.e("区域码：", qym);
							qym = Integer.toHexString(Integer.parseInt(qym)).toUpperCase();
							if(qym.length()%2!=0) {
								qym="0"+qym;
							}
							Log.e("区域码：", qym);
							qym = qym.substring(6) + qym.substring(4,6)+qym.substring(2,4)+qym.substring(0,2);
							
							CRCmsg = ("09"+ "00000000000000"+ "FF07" + pl+qym);
						}
						ordermsg = "5A5A00FE01"+CRCmsg+crc.CRC_CCITT(1,CRCmsg).toUpperCase()+overmsg+ "5B5B/";
						intent.putExtra("order",ordermsg);
						biaotype="newmeter";
						intent.putExtra("metertype",biaotype);
						intent.setClass(AlterBHActivity.this, BtXiMeiService.class);
						startService(AlterBHActivity.this.intent);
